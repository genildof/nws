...
module Keymile
  ...
  class Milegate
    ...
    def get_interface_alarms
      ...
      get_cards_by_name(/COGE/).each do |slot|
        slot.state =~ /Ok/ ?
      ...
      interfaces.each do |interface|
        admin_status = get_interface_admin_status(interface)
        ...
        if admin_status =~ /Up/
          ...
          if sample.scan(REGEX_INTERFACE_SUPPORT)[0].to_s =~ /Supported/
            ...
            if (lines[0].to_i < MAX_OPTICAL_THRESHOLD) & (lines[0].to_i > MIN_OPTICAL_THRESHOLD)
              result << ['INTERFACE', interface, "Low RxInputPower (#{lines[0]})",
    ...
    def get_system_alarms
      ...
      sample.scan(REGEX_SYSTEM_ALARM).each do |line|
        next unless line.scan(REGEX_SYSTEM_ALARM_STATE)[0] =~ /On/ # If alarm is On
      ...
      sample.scan(REGEX_EXTERNAL_ALARMS).each do |line|
        values = line.split(/\|/)
        msg = ''

        next if values[4] =~ /Cleared/ # if present
    ...
    def get_all_cards
      ...
      sample.scan(REGEX_SLOT_LINES).each do |line|
        values = line.split(/\|/).map { |e| e || 0 } # Replaces nil values of array
    ...
    def get_card_alarms
      result = []
      alarmed_cards = get_all_cards.reject { |slot| slot.state.to_s.match(/Ok/) }

      alarmed_cards.each do |card|
        if card.name.to_s =~ /COGE/
          prior = 'Minor'
    ...
    def get_shdsl_params(slot, port)
      ...
      sample.scan(REGEX_DSL_VALUES).each do |value|
        ...
        result << (value.to_s.scan(/\w+/)[0]).to_s
      end

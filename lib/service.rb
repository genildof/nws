...
module Service
  ...
  def get_dmsw_excel_list
    filename = '../config/DMSW.xlsx'
    xlsx = Roo::Spreadsheet.open(filename)
    sheet = xlsx.sheet(0)
    sheet.parse(sub_ring: 'Sub-Anel', hostname: 'Elemento', type: 'HEADEND',
                ip: 'IP', custome: 'CLIENTE', ring_name: 'ANEL', city: 'CIDADE')
  end
  ...
  class ThreadPool
    def self.process!(data, size = 2, &block)
      Array(data).each_slice(size) do |slice|
        slice.map { |item| Thread.new { block.call(item) } }.map(&:join)
      end
    end
